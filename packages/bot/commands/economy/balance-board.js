...
const { getCachedLeaderboardData, invalidateLeaderboardCache } = require('../../utils/cache/balance.cache');
...
const updateLeaderboardValues = (leaderboardData, balanceType) => {
    ...
    return { leaderboardPages, amount: leaderboardValues.length, maxpages: Math.max(0, leaderboardPages.length - 1) };
};
...
const updateLeaderboardEmbed = (interaction, leaderboardPages, amount, page, maxpages, balanceType) => {
    ...
    const description = amount > 0 && leaderboardPages[page]
...
const updateLeaderboardComponents = (leaderboardPages, page, maxpages, balanceType) => {
    ...
    return new ActionRowBuilder().addComponents([...paginationButtons, balanceButton]);
};
...
module.exports.run = async (client, interaction, balanceType = "wallet", page = 0) => {
    ...
    const messageEmbed = updateLeaderboardEmbed(interaction, leaderboardPages, amount, page, maxpages, balanceType);
    const messageComponents = updateLeaderboardComponents(leaderboardPages, page, maxpages, balanceType);
    ...
    collector.on('collect', async i => {
        try {
            ...
            const updatedEmbed = updateLeaderboardEmbed(interaction, updatedPages, updatedAmount, page, updatedMaxPages, balanceType);
            const updatedComponents = updateLeaderboardComponents(updatedPages, page, updatedMaxPages, balanceType);
