...
module.exports.run = async (client, interaction, leaderboard = []) => {
  ...
  if (result?.status === 404) {
    ...
    // If the status code is not 200, return an error that something went wrong
    await followUpInteraction(interaction, {
      content:
        "Oops! Something went wrong while trying to fetch the leaderboard!",
      flags: MessageFlags.Ephemeral,
    });
    return;
  }
  ...
  const leaderboardValues = leaderboard.map((level, index) => {
    ...
    const leaderboardValue = `*Level ${
      level.level
    } / ${level.experience.toLocaleString()} EXP*`;
    return {
  ...
  const messageEmbed = createCustomEmbed({
    ...
    description: `${
      hasLeaderboard
        ? `Here are the top ${leaderboardValues.length} users on the leaderboard!`
        : "Uh oh! There are no users on the leaderboard yet!"
    }`,
  ...
  const paginationButtons =
    leaderboardPages.length > 1
      ? [ClientButtonsEnum.PREVIOUS_PAGE, ClientButtonsEnum.NEXT_PAGE]
      : [];
  ...
  const previousIndex = messageComponents?.components?.findIndex(
    (button) => button.data.custom_id === "previous_pg"
  );
  const nextIndex = messageComponents?.components?.findIndex(
    (button) => button.data.custom_id === "next_pg"
  );
  messageComponents?.components[previousIndex]?.data &&
    (messageComponents.components[previousIndex].data.disabled = true);
  messageComponents?.components[nextIndex]?.data &&
  ...
  collector.on("collect", async (i) => {
    ...
    if (selectedButton === "previous_pg" || selectedButton === "next_pg") {
      ...
      messageEmbed.setFooter({
        text: `Leaderboard page ${page + 1} of ${maxpages + 1}`,
      });
