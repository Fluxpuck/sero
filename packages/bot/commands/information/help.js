...
module.exports.run = async (client, interaction) => {
  ...
  const categoryEmbedFields = [...sortedCommands.entries()].map(
    ([key, value]) => ({
      name: capitalize(key),
  ...
  const messageEmbed = createCustomEmbed({
    ...
    description:
      "Use the dropdown menu to get information on specific categories.",
    fields: categoryEmbedFields,
  });

  // Set a category list for the dropdown
  const categoryListDropdown = [...sortedCommands.entries()].map(
    ([key, value]) => ({
  ...
  collector.on("collect", async (i) => {
    ...
    messageEmbed.data.title = `${client.user.username} | ${capitalize(
      selectedCategory
    )}`;
