  ...
  switch (interaction.customId) {
    case "claim-exp-reward":
      ...
      if (!payload) {
        ...
        try {
          ...
          return interaction.followUp({
            content:
              "This reward drop is no longer available. It may have expired or the bot was restarted.",
      ...
      if (claimed) {
        ...
        if (
          !activeMemberCollection ||
          !activeMemberCollection.includes(interaction.member.id)
        ) {
          return interaction.followUp({
            content:
              "You've not been active enough to claim this reward! Try to be more active next time.",
            flags: MessageFlags.Ephemeral,
          });
        } // Check if the user has already claimed plenty rewards
        const userClaimedInfo =
          previousClaimedCollection &&
          previousClaimedCollection.find?.(
            (item) => item.userId === interaction.member.id
          );
      ...
      try {
        ...
        const result = await postRequest(
          `/guilds/${interaction.guildId}/levels/exp/${interaction.member.id}`,
          { experience: targetAmount }
        );
        ...
        if (result?.status === 200) {
          ...
          const claim_reward_message = CLAIM_MESSAGES[text_idx]
            .replace("{{USER}}", `<@${interaction.member.id}>`)
