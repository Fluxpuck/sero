...
module.exports = async (client, oldMember, newMember) => {
  ...
  if (
    ...
    try {
      ...
      const content = `<t:${unixTimestamp()}> - **${
        oldMember.displayName
      }** changed their name to **${newMember.displayName}**`;
  ...
  if (
    ...
    try {
      ...
      const isTimeout =
        Boolean(newMember.communicationDisabledUntilTimestamp) &&
        Boolean(timeoutChange);
      ...
      const duration = Math.ceil(
        differenceInMinutes(timeoutDate, createdAt) + 1
      );

      // Validate timeout duration (0-10081 minutes / 1 week)
      if (isTimeout && (duration <= 0 || duration > 10081)) {
        console.error(
          `Invalid timeout duration (${duration} minutes) for user ${oldMember.id}`
        );
        return;
      }

      const executorName = executor?.bot
        ? "System"
      ...
      const content = isTimeout
        ? `<t:${currentTime}> - **${
            target.displayName
          }** was timed out for ${duration} ${
            duration === 1 ? "minute" : "minutes"
          } until <t:${timeoutUntil}> by ${executorName}${
            reason ? ` - \`${reason}\`` : ""
          }`
      ...
      console.error(
        `Error processing timeout for user ${oldMember.id}:`,
        error
      );
    }
  }
