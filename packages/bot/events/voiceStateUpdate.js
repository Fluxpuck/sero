...
function checkVoiceSessionsForXp(client) {
    ...
    voiceSessionQueue.forEach((session, userId) => {
        const { member, guild, validForXP, joinTimestamp, lastXpAward = 0 } = session;
        ...
        if (validForXP && (now - lastXpAward) >= VOICE_XP_INTERVAL) {
            ...
            if (process.env.NODE_ENV === "development") {
                console.log("\x1b[36m", `[Client]: ${member.user.tag} gained XP from ongoing voice activity`);
            }
        }
    });
}
...
async function handleVoiceSessionEnd(session, client) {
    const { joinTimestamp, leaveTimestamp, member, channel, guild, validForXP } = session;
    ...
    if (duration >= 60) {
        ...
        if (validForXP && duration >= 60000 && duration < VOICE_XP_INTERVAL) {
            ...
            if (process.env.NODE_ENV === "development") {
                console.log("\x1b[36m", `[Client]: ${member.user.tag} gained final XP from voice session`);
            }
        }
    }
...
module.exports = async (client, previousState, newState) => {
    ...
    if (!voiceXpInterval) {
        voiceXpInterval = setInterval(() => checkVoiceSessionsForXp(client), VOICE_XP_INTERVAL);
    ...
    if (!previousState.channel && newState.channel) {
        ...
        if (process.env.NODE_ENV === "development") {
            console.log("\x1b[36m", `[Client]: ${member.user.tag} joined ${newState.channel.name} (XP eligible: ${validForXP})`);
        }
    ...
    // User switched voice channels or state changed (mute/unmute, etc.)
        ...
        if (newState.channel && previousState.channel && previousState.channel.id !== newState.channel.id) {
            if (session) {
                session.leaveTimestamp = Date.now();
                ...
                if (process.env.NODE_ENV === "development") {
                    console.log("\x1b[36m", `[Client]: ${member.user.tag} switched from ${previousState.channel.name} to ${newState.channel.name} (XP eligible: ${validForXP})`);
                }
            }
        }
        // If state changed (mute/unmute or people joining/leaving the channel)
            ...
            if (session.validForXP !== currentValidForXP) {
                ...
                if (process.env.NODE_ENV === "development") {
                    console.log("\x1b[36m", `[Client]: ${member.user.tag}'s XP eligibility changed to: ${currentValidForXP}`);
                }
            }
        }
    }
