...
export class DiscordFetchMessagesTool extends ClaudeToolType {
    static getToolContext() {
        return {
            ...
            description: "Fetch messages from a specific Discord channel or user. The tool can filter messages by type (gif, url, media, sticker) and time range. Only use this tool if explicitly requested by the user.", input_schema: {
                ...
                properties: {
                    ...
                    messageId: {
                        type: "string",
                        description: "The ID of a specific message to fetch",
                    },
                    limit: {
                        type: "number",
                    ...
                    mode: {
                        type: "string",
                        enum: ["user", "channel", "auto", "single"],
                        description: "The mode of fetching messages. 'user' for user messages, 'channel' for channel messages, 'single' for specific message by ID, and 'auto' for automatic detection.",
                    },
                },
                required: ["inChannel"],
            },
    ...
    } async execute({ fromUser, inChannel, limit = 100, mode = "auto", messageId }: FetchMessagesInput): Promise<string> {
        if (!this.message.guild) {
            return `Error: This command can only be used in a guild.`;
        }

        try {
            // Set mode to single if messageId is provided
            if (messageId) {
                mode = "single";
            }
