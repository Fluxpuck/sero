...
export class DiscordModerationTool extends ClaudeToolType {
  static getToolContext() {
    return {
      ...
      description:
        "Find and optionally moderate a Discord user with various actions",
      input_schema: {
        ...
        properties: {
          ...
          actions: {
            ...
            description:
              "Array of optional moderation actions to perform on the user: Timeout (mute), Disconnect (from voice), Kick, Ban, Warn (DM), Verbal-Warning (in channel)",
          },
          timeout_duration: {
            type: "number",
            description:
              "Duration in minutes for timeout (ignored for other actions)",
          },
          reason: {
            type: "string",
  ...
  }: ModerationToolInput): Promise<string> {
      ...
      try {
        switch (action) {
          case "timeout":
            if (!timeout_duration) {
              throw new Error(
                `Timeout duration is required for timeout action.`
              );
            }
            await user
              .timeout(timeout_duration * 60 * 1000, fullReason)
              .catch((error) => {
          ...
          case "kick":
            if (!this.message.member?.permissions.has("KickMembers")) {
              throw new Error(
                `This user does not have permission to kick members.`
              );
            }
            if (!user.kickable) {
            ...
            await user.kick(fullReason).catch((error) => {
              throw new Error(
                `Failed to kick user "${user.user.tag}". Reason: ${error}`
              );
            });
          ...
          case "ban":
            if (!this.message.member?.permissions.has("BanMembers")) {
              throw new Error(
                `This user does not have permission to ban members.`
              );
            }
            if (!user.bannable) {
            ...
            await user
              .ban({ deleteMessageSeconds: 24 * 60 * 60, reason: fullReason })
              .catch((error) => {
                throw new Error(
                  `Failed to ban user "${user.user.tag}". Reason: ${error}`
                );
              });
          ...
          case "verbal-warning":
            ...
            await replyOrSend(
              this.message,
              `<@${user.id}>, ${customMessage}`
            ).catch((error) => {
              throw new Error(
                `Failed to send verbal warning in channel. Reason: ${error}`
              );
            });
        ...
        throw new Error(
          `Failed to ${action} ${user.user.tag}: ${
