...
import { GenerateImageTool } from "../tools/openai_image_generation.tool";
...
export function initializeTools(message: Message, client: Client) {
  toolInstances.set(
    "discord_guild_info",
    new DiscordGuildInfoTool(client, message)
  );
  toolInstances.set(
    "discord_fetch_messages",
    new DiscordFetchMessagesTool(client, message)
  );
  toolInstances.set(
    "discord_moderation_actions",
    new DiscordModerationTool(client, message)
  );
  toolInstances.set(
    "discord_send_message",
    new DiscordSendMessageTool(client, message)
  );
  toolInstances.set("generate_image", new GenerateImageTool(client, message));
}
