...
const findAllRecords = async (model, options, timeout = DEFAULT_TIMEOUT_MS) => {
    try {
        ...
        return new CreateError(408, `Timeout request on findAllRecords ${model.name}`);
    }
};
...
const findOneRecord = async (model, options, timeout = DEFAULT_TIMEOUT_MS) => {
    try {
        ...
        return new CreateError(408, `Timeout request on findOneRecord ${model.name}`);
    }
};
...
const createOrUpdateRecord = async (model, data, transaction, timeout = DEFAULT_TIMEOUT_MS) => {
    try {
        ...
        return new CreateError(408, `Timeout request on createOrUpdateRecord ${model.name}`);
    }
};
...
const createUniqueRecord = async (model, data, transaction, timeout = DEFAULT_TIMEOUT_MS) => {
    try {
        ...
        return new CreateError(408, `Timeout request on createUniqueRecord ${model.name}`);
    }
};
